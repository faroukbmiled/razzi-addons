...
class Product_Category_Tabs extends Widget_Base
{
	...
	protected function section_heading_settings()
	{
		...
		$this->add_control(
			'top_level_only',
			[
				'label' => esc_html__('Top level categories only', 'razzi'),
		...
		$this->add_control(
			'header_tabs_display',
			[
				'label' => esc_html__('Display Header Tabs', 'razzi'),
				'type' => Controls_Manager::SWITCHER,
	...
	protected function render()
	{
		...
		if ($settings['product_cat']) {
			...
			if ('yes' === $settings['header_tabs_display']) {
				$header_tabs_class = 'tabs-header tabs-nav tabs';
			} else {
				$header_tabs_class = 'd-none';
			}

			$header_tabs[] = '<ul class="' . $header_tabs_class . '">';
			$i = 0;
			foreach ($cats as $tab) {
				...
				$tab_content[] = sprintf(
					'<div class="tabs-panel tabs-%s tab-loaded %s"><div class="tab-content swiper-container linked-products-category">%s</div></div>',
					esc_attr($tab),
					$class_active,
			...
			$output[] = implode('', $header_tabs);
			$output[] = '<div class="tabs-content">';
			$output[] = '<div class="tabs-content-wrapper swiper-wrapper">';
			$output[] = implode('', $tab_content);
			$output[] = '</div></div>';
		} else {
